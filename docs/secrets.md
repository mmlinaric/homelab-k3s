...
For Jenkins, use a confidential client with client authentication enabled, the standard authorization-code flow enabled, direct access grants disabled, and required PKCE method `S256`. Set the web origin to `https://jenkins.mmlinaric.com`; add both `https://jenkins.mmlinaric.com/` and `https://jenkins.mmlinaric.com/OicLogout` as valid post-logout redirect URIs. Add an `admin` client role assigned to the `homelab-admins` group. Add a User Client Role mapper that emits the Jenkins client roles as the multivalued `jenkins_roles` claim in ID tokens, access tokens, and UserInfo. Jenkins rejects login unless that claim contains `admin`. Jenkins uses one Forgejo access token owned by `mmlinaric`, scoped to `write:repository,read:user,read:organization`, for organization-folder discovery, automatic repository webhooks, HTTPS checkout, and commit statuses. The Gitea plugin probes the organization API before falling back to a personal user namespace, so `read:organization` is required even when all repositories are owned directly by `mmlinaric`. Store the token as `jenkins-forgejo-api-token` in Bitwarden; the Jenkins ExternalSecret and JCasC credential consume it without creating per-repository credentials. The `Forgejo · mmlinaric` Organization Folder creates jobs only for non-archived repositories whose selected branch contains a root-level `Jenkinsfile`; migrated GitLab CI definitions are not Jenkins pipelines and must be converted deliberately.
...
For Forgejo, use a confidential client with client authentication enabled, the standard authorization-code flow enabled, direct access grants disabled, and required PKCE method `S256`. Forgejo 15's OpenID Connect client automatically sends an S256 code challenge and the matching verifier. Set the web origin to `https://git.mmlinaric.com`. Add client roles named `user` and `admin`; make `admin` a composite client role that includes `user`. Assign only `admin` to `homelab-admins`; no other realm group is permitted to use this deployment. Add a User Client Role mapper that emits the expanded client roles as the multivalued `forgejo_roles` claim in ID tokens, access tokens, and UserInfo. The mapper claim is not an OAuth scope; Forgejo requests only `openid email profile`. Forgejo rejects login without `user` and grants site administration when `admin` is present. Automatic OIDC registration is disabled: after Keycloak authenticates and authorizes a new user, Forgejo asks them to select their permanent local username without creating a local password. The authentication source does not enable `allow-username-change`, so external users cannot rename themselves later. Internal and password sign-in are disabled, and unauthenticated users are redirected to the Keycloak-only login page. The local bootstrap administrator is retained for CLI recovery; temporarily re-enabling internal sign-in through GitOps is required if it must be used through the web interface during a Keycloak outage. Forgejo's OCI registry uses its native instance hostname: authenticate with `docker login git.mmlinaric.com` using the Forgejo username and a personal access token, and name images `git.mmlinaric.com/<owner>/<image>:<tag>`. The existing GitLab registry remains at `registry.mmlinaric.com`.

The five Forgejo Bitwarden entries are referenced by UUID in `apps/forgejo/secrets.yaml`. Generate the database, OIDC, and bootstrap administrator passwords with at least 32 random bytes. Rotate entry values in place where possible; update the manifest UUID only when an entry is recreated.
